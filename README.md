...
- **How it polls and stops:** GitHub's scheduled cron throttles frequent schedules badly, so one workflow run **polls internally** — reply, sleep 10 min, repeat — during the active windows (`BOT_ACTIVE_WINDOWS`, Cairo; e.g. `20-2,4-10` = **8 PM–2 AM and 4 AM–10 AM**), replying to your newest post up to `BOT_PER_POST_CAP` (**200**), then stopping. Because a single cloud job maxes out at 6 hours, it **hands off to a fresh run** near that limit so coverage spans a window. Progress (`state.json`) is committed every cycle, so a restart never double-replies. The `*/30` cron is a backup restart; for a **prompt start right after you post**, trigger it from **Actions → Run workflow**.
