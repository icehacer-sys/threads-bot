...
4. Send each comment to the model **with the answer, the vetted facts, and the images as context** (the post's X-ray, plus any image the commenter attached or a still frame pulled from their GIF or video), so a correct guess gets "Spot on ✅", a wrong guess gets a kind fact-based nudge, a question gets a short accurate answer, and a joke (or a meme image) gets one back.
...
The key is the post's **shortcode**, the code at the end of its URL (`threads.com/@mdnoteslab/post/DZX4-v_k7lT`). `aliases` are other ways people might phrase the right answer. When a comment matches, the bot affirms it ("Spot on ✅" or "You nailed it ✅"); when it's a joke, it jokes back; it never reveals the answer to a wrong guess.
...
Prefer to hide the answer behind a tap instead? Set `BOT_ANSWER_SPOILER=on`: only the `Answer:` label stays visible and the diagnosis plus the whole explanation are blurred until tapped. Just start your breakdown with `Answer: <diagnosis>` (the normal format), no teaser line needed.
...
2. Add the **Threads API** use case and enable these permissions or scopes:
   - `threads_basic`
   - `threads_content_publish`
   - `threads_manage_replies` (read and reply to comments)
...
Long-lived tokens expire (about 60 days) and need refreshing, so note that for later.

## Running it on a schedule (cloud, 24/7)

`npm run live` is one pass. A ready-to-use **GitHub Actions** workflow ships at `.github/workflows/reply.yml`. It polls every ~10 minutes but only acts during your active window (currently **10 PM to 10 AM Cairo**, set by `BOT_ACTIVE_WINDOWS`). The shipped workflow also enables vision and web search and uses caps of 200 per post and 240 per day. It runs on GitHub's servers, so your PC does not need to be on.
...
1. Create a **private** GitHub repo and push this project to it:
   ...
   ```
   `.env` is gitignored, so your secrets are **not** pushed. `state.json` **is** pushed, which is intentional, so the bot remembers what it answered.
2. In the repo, open Settings, then Secrets and variables, then Actions, then New repository secret, and add two secrets: `ANTHROPIC_API_KEY` and `THREADS_ACCESS_TOKEN`.
...
- `BOT_WINDOW_HOURS`: only reply within the first N hours of a post (default 0, meaning no limit; rely on the per-post cap).
...
- `BOT_DAILY_CAP`: max replies per 24h backstop (default 200). Threads allows ~250 published per day and replies count, so stay under.
- `BOT_PER_POST_CAP`: hard cap on total replies per post, counted across runs (default 100).
- `BOT_EDUCATIONAL`: `on` (default) means once the answer is known, it corrects wrong guesses and answers questions using the vetted `facts`. `off` leaves those for you.
- `BOT_VISION`: `on` (default) sends the post's X-ray image to the model so it can see the case.
...
- `BOT_ANSWER` and `BOT_ANSWER_DELAY_HOURS`: auto-post the answer breakdown after N hours (default on, 1h), but **only for posts that have a `breakdown` in `data/answers.json`**, otherwise you post and pin the answer yourself. Either way you pin it manually.
- `BOT_ACTIVE_WINDOWS`: the active posting window(s) in `BOT_ACTIVE_TZ`, as comma-separated `start-end` hour pairs that may wrap past midnight (currently `22-10`, meaning 10 PM to 10 AM Cairo; e.g. `20-2,4-10` for two windows). This is the canonical setting.
- `BOT_ACTIVE_TZ`, `BOT_ACTIVE_START`, `BOT_ACTIVE_END`: timezone (e.g. `Africa/Cairo`) plus a single-window fallback used only when `BOT_ACTIVE_WINDOWS` is unset. Timezone-aware and DST-safe. Empty TZ means always on.
...
- `claude-sonnet-4-6`: best balance of wit and cost (about $10 to $15 a month at 100 replies a day). Default.
- `claude-haiku-4-5`: cheapest that still works (about $4 to $6 a month), a touch less clever on the jokes.
...
- Personal medical questions ("could I have this", "should I get scanned") are **always skipped**, in two layers: the model classifies them as `personal_medical`, and `sanitize()` in `src/reply.ts` force-skips anything that still reads like advice.
- Complaints and spam are skipped (a human handles complaints).
- Any error or unparseable response is skipped. The bot defaults to silence, never to a risky reply.
- It stays humble: it replies peer-to-peer to obvious clinicians instead of lecturing, never over-claims anatomy beyond the vetted facts, and concedes in one line or skips rather than arguing when a comment challenges it.

## Files

- `src/voice.ts`: the brand voice and few-shot examples (edit this to tune replies).
- `src/reply.ts`: Claude call and safety sanitizer.
- `src/threads.ts`: all Threads API calls.
- `src/index.ts`: orchestration and the three run modes.
- `src/state.ts`: local replied-log and daily counter.
