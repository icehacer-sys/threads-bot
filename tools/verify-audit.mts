...
assert.throws(() => validateNotes(notes, "max_tokens"), /stop_reason=max_tokens/);
assert.throws(() => validateNotes(notes + "\n- x".repeat(17), "end_turn"), /bullets=20/);
assert.throws(() => validateNotes(notes + '\n- ' + 'x'.repeat(500), 'end_turn'), /longest bullet=502/);
assert.throws(() => validateNotes(notes.replace('## Retire', '## Other'), 'end_turn'), /missing heading: ## Retire/);
assert.throws(() => validateNotes(notes + 'x'.repeat(6000), 'end_turn'), /characters=/);
