...
const {replyCoverage,neutralGuessAcknowledgment,isNeutralGuessAcknowledgment}=await import('../src/reply-coverage');
...
for(const secret of ['Congenital knee dislocation','An unrelated diagnosis']) {
 queue=[{...base,intent:'diagnosis guess',category:'affirm',reply_text:'Exactly right!'}];calls=0;
 const result=await classifyAndDraft({...input,answer:secret,answerPublic:false,commentText:'Dislocation?'});
 assert.equal(result.reply_text,neutralGuessAcknowledgment('Dislocation?'));assert.equal(calls,1);
 assert.ok(isNeutralGuessAcknowledgment(result.reply_text));assert.equal(JSON.stringify(body).includes(secret),false,'secret answer is not sent before reveal');
}
queue=[{...base,intent:'diagnosis guess',decision:'skip',reply_text:'',reason:'Diagnosis guess before reveal'}];
assert.ok(isNeutralGuessAcknowledgment((await classifyAndDraft({...input,answerPublic:false})).reply_text));
